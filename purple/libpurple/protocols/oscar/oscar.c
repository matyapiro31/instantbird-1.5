...
connection_common_established_cb(FlapConnection *conn)
{
	...
	{
		if (purple_account_get_bool(account, "use_clientlogin", od->icq ? ICQ_DEFAULT_USE_CLIENTLOGIN : AIM_DEFAULT_USE_CLIENTLOGIN))
		{
		...
		gconstpointer value, gpointer data)
{
	...
	report_idle = GPOINTER_TO_INT(value);
...
oscar_login(PurpleAccount *account)
{
	...
	purple_prefs_connect_callback(gc, "/messenger/status/reportIdle", idle_reporting_pref_cb, gc);
...
static int purple_ssi_parselist(OscarData *od, FlapConnection *conn, FlapFrame *fr, ...)
{
	...
	if (tmp != 0xFFFFFFFF) {
		...
		report_idle = purple_prefs_get_bool("/messenger/status/reportIdle");
			...
			case AIM_SSI_TYPE_BUDDY: { /* Buddy */
				if (curitem->name) {
					...
					if (b) {
						...
						if (!purple_strequal(curitem->name, alias_utf8))
...
void oscar_init(PurplePlugin *plugin, gboolean is_icq)
{
	...
	option = purple_account_option_bool_new(_("Use clientLogin"), "use_clientlogin",
			is_icq ? ICQ_DEFAULT_USE_CLIENTLOGIN : AIM_DEFAULT_USE_CLIENTLOGIN);
	prpl_info->protocol_options = g_list_append(prpl_info->protocol_options, option);
