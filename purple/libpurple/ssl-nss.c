...
ssl_nss_connect(PurpleSslConnection *gsc)
{
	...
	SSL_OptionSet(nss_data->in, SSL_ENABLE_SESSION_TICKETS, PR_FALSE);

	if (!purple_prefs_get_bool("/purple/ssl/check_certificates"))
...
ssl_nss_read(PurpleSslConnection *gsc, void *data, size_t len)
{
	...
	if (nss_data->handshake_handler != 0) {
		purple_debug_error("nss", "attempting to read from an ssl socket before the end of the handshake\n");
		return -1;
	}
...
ssl_nss_write(PurpleSslConnection *gsc, const void *data, size_t len)
{
	...
	if (nss_data->handshake_handler != 0) {
		purple_debug_error("nss", "attempting to write to an ssl socket before the end of the handshake\n");
		return -1;
	}
...
#if 0
/* No purple_certificate_* stuff for Instantbird */
...
void purple_nss_init(void)
{
	purple_ssl_set_ops(&ssl_ops);
}

#if 0
/* Instantbird does not need the plugin stuff for NSS */
static gboolean
